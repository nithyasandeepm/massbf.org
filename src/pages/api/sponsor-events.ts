...
async function read(): Promise<any[]> {
  return readDataFile("sponsor-events", []);
}

async function save(data: any[]) {
  await writeDataFile("sponsor-events", data);
}
...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (body.type === "event") {
    ...
    if (!name)
      return new Response(JSON.stringify({ error: "Event name required" }), { status: 400, headers: { "Content-Type": "application/json" } });
  ...
  if (body.type === "sponsor") {
    ...
    if (!(name || "").trim())
      return new Response(JSON.stringify({ error: "Sponsor name required" }), { status: 400, headers: { "Content-Type": "application/json" } });
...
export const PUT: APIRoute = async ({ request }) => {
  ...
  if (body.type === "sponsor") {
    ...
    if (si === -1)
      return new Response(JSON.stringify({ error: "Sponsor not found" }), { status: 404, headers: { "Content-Type": "application/json" } });
